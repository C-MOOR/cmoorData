## code to prepare `marianes-htseq` dataset goes here

meta <- read.table("data-raw/marianes-htseq/midgut.tsv", header=TRUE, sep="\t")
...
for (row in 1:nrow(meta)) {
  ...
  data  <- read.table(paste0("data-raw/marianes-htseq/", meta[row, "filename"]), sep="\t")
